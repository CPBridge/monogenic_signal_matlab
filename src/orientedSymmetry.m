function [FA_y, FA_x, FS] = orientedSymmetry(m1,m2,m3,T)
%
%  [FA_y, FA_x, FS] = orientedSymmetry(m1,m2,m3,T = 0.18)
%
%   Creates an edge map using oriented feature asymmetry calculated from
%   the monogenic signal (m1,m2,m3). FA_y contains the vertical edge
...
%   
%   FS returns the signed feature symmetry map (i.e. feature symmetry
...
even = abs(m1);
odd = sqrt(m2.*m2 + m3.*m3);

% Calculate the denominator (= local amplitude + epsilon)
denominator = sqrt(even.*even + m2.*m2 + m3.*m3) + epsilon;
